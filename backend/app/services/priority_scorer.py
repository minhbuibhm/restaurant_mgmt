class PriorityScorer:
    """Ranks kitchen items using wait time, complexity, and station load."""
    ...
    def calculate_score(
        ...
        return round(
            self.wait_time_weight * self.weigh_wait_time(wait_seconds)
            + self.complexity_weight * self.weigh_complexity(prep_time_minutes)
            + self.station_load_weight * self.weigh_station_load(station_active_count),
            1,
        )
